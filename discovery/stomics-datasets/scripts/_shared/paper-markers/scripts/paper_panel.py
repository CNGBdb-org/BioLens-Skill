#!/usr/bin/env python3
"""Plot paper curated gene panel spatial maps."""
...
from gene_tables_bridge import paper_panels as _panels
from io_core import validate_dataset_dir
from plot_spatial_gene import plot_gene


def main():
    PAPER_PANELS = _panels()
    p = argparse.ArgumentParser(description="Paper gene panel spatial plots.")
    ...
    data_dir = args.data_dir or theme.get("default_sample")
    if not data_dir:
        print("Provide data_dir for this theme.", file=sys.stderr)
        sys.exit(1)
