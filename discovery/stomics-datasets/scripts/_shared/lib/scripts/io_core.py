"""CNGB Cirrocumulus parquet I/O (atlas-agnostic; configure via atlas_registry)."""
...
class Dataset:
    """Local folder, remote CNGB explore parquet, or cached h5ad (CDCP)."""
    ...
    def __init__(
        ...
        *,
        ...
        self.h5ad_url = h5ad_url or (getattr(meta, "download", "") if meta else "")
        self._schema = None
        self._h5ad = None
    ...
    def h5ad_path(self) -> str:
        if self.mode == "local" and self.local_root:
            for name in (H5AD_CACHE_NAME, "data.h5ad"):
                p = os.path.join(self.local_root, name)
                if os.path.isfile(p):
                    return p
            for fn in os.listdir(self.local_root):
                if fn.endswith(".h5ad"):
                    return os.path.join(self.local_root, fn)
        return os.path.join(self.cache_dir(), H5AD_CACHE_NAME)

    def _get_h5ad(self):
        if self._h5ad is None:
            from h5ad_core import H5adHandle, ensure_h5ad

            path = ensure_h5ad(self.h5ad_path(), self.h5ad_url)
            self._h5ad = H5adHandle(path)
        return self._h5ad

    def embedding_axis_labels(self) -> tuple[str, str]:
        if self.mode == "h5ad":
            return self._get_h5ad().embedding_axis_labels()
    ...
    def exists(self, rel_path):
        if self.mode == "h5ad":
            if rel_path == "index.json.gz":
                return True
            if rel_path.startswith("obs/") or rel_path.startswith("X/"):
                return True
            if rel_path == "obsm/spatial.parquet":
                return self._get_h5ad().embedding_key() is not None
            return False
        if self.mode == "local":
    ...
    def read_bytes(self, rel_path):
        if self.mode == "h5ad":
            raise FileNotFoundError(f"h5ad backend has no byte file: {rel_path}")
        if self.mode == "local":
    ...
    def list_genes(self):
        if self.mode == "h5ad":
            return self._get_h5ad().var_names()
...
def resolve_dataset(data_dir):
    ...
    if use_local and os.path.isdir(local):
        ...
        for fn in os.listdir(local):
            if fn.endswith(".h5ad"):
                try:
                    dataset_id, meta = resolve_dataset_ref(os.path.basename(fn))
                except (FileNotFoundError, ValueError):
                    dataset_id, meta = os.path.basename(fn), None
                return Dataset(
                    dataset_id,
                    mode="h5ad",
                    local_root=local,
                    meta=meta,
                    h5ad_url=getattr(meta, "download", "") if meta else "",
                )
    ...
    # Prefer API over multi-GB h5ad download when the endpoint responds.
    if _parquet_explore_available(dataset_id):
        return Dataset(dataset_id, mode="api", meta=meta, explore_id=dataset_id)

    if download or kind in ("h5ad", "scrna_h5ad") or spec.name.upper().startswith("SCDS"):
        if not download and meta and meta.section.endswith(".h5ad"):
            raise FileNotFoundError(
                f"No explore API and no Download URL for {dataset_id}. Re-run bootstrap_cdcp.py."
            )
        return Dataset(dataset_id, mode="h5ad", meta=meta, h5ad_url=download)
...
def load_schema(ds):
    if ds.mode == "h5ad":
        return ds._get_h5ad().pseudo_schema()
...
def list_obs_columns(ds, *, check_exists=False):
    ...
    if ds.mode == "h5ad":
        return ds._get_h5ad().obs_columns()
...
def resolve_obs_column(ds, column: str | None = None) -> str:
    ...
    if ds.mode == "h5ad":
        h = ds._get_h5ad()
        requested = (column or get_atlas().default_obs_label or h.default_obs_column()).strip()
        if requested == "__all__" or not h.obs_columns():
            return "__all__"
        cols = h.obs_columns()
...
def load_obs_column(ds, column):
    ...
    if ds.mode == "h5ad":
        return ds._get_h5ad().load_obs(column)
...
def load_spatial_xy(ds, use_cache=True):
    if ds.mode == "h5ad":
        cache_path = os.path.join(ds.cache_dir(), "obsm", SPATIAL_CACHE)
        if use_cache and os.path.exists(cache_path):
            return np.load(cache_path, mmap_mode="r")
        xy = ds._get_h5ad().load_xy(cache_path if use_cache else None)
        return xy
...
def resolve_gene_name(ds, gene):
    if ds.mode == "h5ad":
        return ds._get_h5ad().resolve_gene(gene, fuzzy=get_atlas().fuzzy_gene_symbols)
...
def gene_in_var(ds, gene):
    if ds.mode == "h5ad":
        return ds._get_h5ad().resolve_gene(gene, fuzzy=get_atlas().fuzzy_gene_symbols) is not None
...
def load_gene_sparse(ds, gene):
    if ds.mode == "h5ad":
        return ds._get_h5ad().load_gene(gene)
