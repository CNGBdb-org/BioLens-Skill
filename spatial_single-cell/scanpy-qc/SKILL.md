...
name: scanpy-qc
description: >-
  Run Scanpy QC on an AnnData/h5ad: QC metrics, violin plots, filter by min genes and mito percent. Use for single-cell quality control before preprocessing. Not for ingest (use sc-ingest), preprocessing/PCA (use scanpy-preprocess), or HESTA maps (use hesta).
...
metadata:
  ...
  depth: L5
  domain: single-cell
  capability_id: cngbdb.scrna.scanpy-qc.v1
...
|------|------|
| 读入矩阵 | `sc-ingest` |
| normalize/HVG/PCA | `scanpy-preprocess` |
| HESTA 图谱 QC | `hesta` |
| 空间 QC | `spatial-qc` |
