...
name: scanpy-cluster
description: >-
  Scanpy neighbors, UMAP, and Leiden clustering on preprocessed AnnData. Use for single-cell cluster discovery. Not for QC/preprocess (use scanpy-qc / scanpy-preprocess) or marker ranking (use scanpy-markers).
...
metadata:
  ...
  depth: L5
  domain: single-cell
  capability_id: cngbdb.scrna.scanpy-cluster.v1
...
- neighbors、UMAP、Leiden 聚类

## Do Not Use When

| 需求 | 交给 |
|------|------|
| 预处理 | `scanpy-preprocess` |
| 找 marker | `scanpy-markers` |
| HESTA 子结构/空间图 | `hesta` |
