...
name: scvi-integrate
description: >-
  Integrate scRNA batches with scvi-tools SCVI when installed; otherwise fall back to Scanpy Combat/PCA. Use for deep generative batch correction. Not for light Combat multi-integrate (use sc-multi-integrate) or single-sample cluster (use scanpy-cluster).
...
metadata:
  ...
  depth: L5
  domain: single-cell
  capability_id: cngbdb.scrna.scvi-integrate.v1
...
- scvi-tools SCVI 批次整合；无包则 Combat 降级

## Do Not Use When

| 需求 | 交给 |
|------|------|
| 轻量 Combat 整合报告 | `sc-multi-integrate` |
...
| 任务 | 必填 |
|------|------|
| `run` | 含 `batch`（或 `--batch-key`）列的 h5ad |
...
```bash
python ./scripts/query.py <h5ad> [-o outdir] [--batch-key batch] [--max-epochs 50]
```
...
python ./scripts/query.py integrated_or_batched.h5ad -o out/scvi
```
