# BioLens-Skill · CIMA Pipeline Skills

[CIMA](https://db.cngb.org/stomics/) / TrueBlood **本地流水线与独立查询 Skill**。本仓库共 **12** 个 Skill，源自 CNGBdb BioLens。

仓库：https://github.com/CNGBdb-org/BioLens-Skill
...
DEST=/path/to/project/.cursor/skills   # 换成上表对应路径
mkdir -p "$DEST"
for d in BioLens-Skill/cima-*/; do
  name=$(basename "$d")
...
### 方式二：通用 CLI `npx skills add`（需 Node.js）

```bash
npx skills add https://github.com/CNGBdb-org/BioLens-Skill --skill cima-scrna-preprocessing -a cursor
npx skills add https://github.com/CNGBdb-org/BioLens-Skill --skill cima-xqtl -a claude-code -a codex
npx skills add https://github.com/CNGBdb-org/BioLens-Skill --skill cima-clm -a cursor
```

### 依赖

各 Skill 依赖见对应 `SKILL.md`，常见包括：

- `scanpy` / `anndata`（流水线）
- `statsmodels`（`cima-pseudobulk-variance`）
- `harmony-pytorch` / `harmonypy`（可选批次校正）
- `cosg`（可选，`cima-cell-annotation`）

## 使用示例

每个 Skill 的 `SKILL.md` 均有 **Examples**。本仓库提供合成 demo（仅冒烟，非论文复现）：
...
```bash
# 详见 demo/README.md：paired_demo_rna + paired_demo_atac(+_gene)
# 1 preprocess → 2 annotate → 3 pseudobulk
# 4 scratac peaks(+gene activity) → 5 multiomics → 6 metacell（可配对）
```
